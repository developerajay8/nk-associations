  ...
  return (
    <section id="process" className="bg-white py-24 px-4 relative overflow-hidden">
      ...
      <div className="absolute inset-0 opacity-[0.025] pointer-events-none"
        style={{
          backgroundImage: "linear-gradient(rgba(55,115,182,0.5) 1px, transparent 1px), linear-gradient(90deg, rgba(55,115,182,0.5) 1px, transparent 1px)",
      ...
      <div ref={ref} className="max-w-7xl mx-auto relative z-10">
        ...
        <div className={`text-center mb-16 transition-all duration-700 ${inView ? "opacity-100 translate-y-0" : "opacity-0 translate-y-8"}`}>
          ...
          <h2 className="text-4xl md:text-5xl font-black text-[#232021] leading-tight mb-4">
            ...
            <span className="text-[#3773b6]">Apna Kaam Karo Done</span>
          </h2>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {steps.map((step, i) => (
            <div
              key={step.num}
              className={`group relative bg-gradient-to-b from-white to-[#3773b6]/5 border border-[#3773b6]/10 rounded-3xl p-7 hover:bg-white hover:border-[#3773b6]/30 hover:shadow-xl hover:shadow-[#3773b6]/8 hover:-translate-y-1.5 transition-all duration-300 ${inView ? "opacity-100 translate-y-0" : "opacity-0 translate-y-8"}`}
              style={{ transitionDelay: `${i * 80}ms` }}
            >
              {/* Step Number */}
              <div className="absolute top-6 right-6 text-6xl font-black leading-none select-none"
                style={{ color: "rgba(55,115,182,0.06)" }}>
                {step.num}
              </div>

              {/* Icon Circle */}
              <div className="w-14 h-14 rounded-2xl bg-[#3773b6]/10 flex items-center justify-center text-2xl mb-5 group-hover:bg-[#3773b6]/15 group-hover:scale-110 transition-all duration-300">
                {step.icon}
              </div>

              {/* Step Label */}
              <div className="flex items-center gap-2 mb-2">
                <span className="text-[10px] font-black tracking-widest text-[#3773b6] uppercase">Step {step.num}</span>
                <span className="flex-1 h-px bg-[#3773b6]/15" />
              </div>

              <h3 className="font-bold text-[#232021] text-lg mb-2">{step.title}</h3>
              <p className="text-[#232021]/55 text-sm leading-relaxed">{step.desc}</p>
            </div>
          ))}
        </div>
        ...
        <div className={`mt-14 flex flex-col sm:flex-row items-center justify-center gap-4 transition-all duration-700 delay-500 ${inView ? "opacity-100 translate-y-0" : "opacity-0 translate-y-8"}`}>
          <div className="text-center sm:text-left">
            <p className="text-[#232021] font-bold text-lg">Shuru karna chahte hain?</p>
            <p className="text-[#232021]/50 text-sm">Step 1 bilkul free hai — abhi book karein</p>
          </div>
          <button
            onClick={() => document.getElementById("contact")?.scrollIntoView({ behavior: "smooth" })}
            className="bg-gradient-to-r from-[#3773b6] to-[#2a5a99] text-white font-bold px-8 py-3.5 rounded-xl text-sm shadow-lg shadow-[#3773b6]/30 hover:-translate-y-0.5 hover:shadow-[#3773b6]/50 transition-all duration-200"
          >
